...
注意，<span style="color:red;"><strong>你不能直接侦听响应式对象的属性值，侦听一个reactive对象的属性需要提供一个返回该属性的 getter 函数</strong></span>，例如:
...
`watchEffect()` 允许我们自动跟踪回调的响应式依赖，而不用显示的指定监听源。
...
同时监听多个值时，`watchEffect()` 的好处就显现出来了。
